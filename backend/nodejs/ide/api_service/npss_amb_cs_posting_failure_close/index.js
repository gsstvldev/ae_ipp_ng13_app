...
app.post('/', function(appRequest, appResponse, next) {
    ...
    try {
        ...
        var serviceName = 'NPSS AMB (CS) Posting failure close';
        ...
        reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
            try {
                ...
                // Log Viewer Setup
                objSessionLogInfo.HANDLER_CODE = 'NPSS AMB (CS) Posting failure close';
                objSessionLogInfo.ACTION = 'ACTION';
                objSessionLogInfo.PROCESS = 'NPSS AMB (CS) Posting failure close';
                // Get DB Connection                                                                                                                                      
                        ...
                        try {
                            ...
                            ExecuteQuery1(take_status, function (arrrule) {
                                if (arrrule.length > 0) {
                                    ...
                                    var TakeTrnData = `select * from NPSS_CAMT_ACC_CRDR_NTFN   where ncacn_id in ${TempTranID}`
                                    ExecuteQuery1(TakeTrnData, function (arrprocesslog) {
                                        if (arrprocesslog.length > 0) {
                                            var arrCusTranInst = [];
                                            for (let i = 0; i < arrprocesslog.length; i++) {
                                                var objCusTranInst = {};
                                                objCusTranInst.MSG_ID = arrprocesslog[i].header_msg_id ;
                                                objCusTranInst.PRCT_ID = PRCT_ID;
                                             
                                                objCusTranInst.PROCESS_REF_NO = arrprocesslog[i].end_to_end_id ;
                                                ...
                                                objCusTranInst.PROCESSING_SYSTEM = '';
                                            ...
                                            _BulkInsertProcessItem(arrCusTranInst, 'NPSS_CAMT_PROCESS_LOG', function callbackInsert(CusTranInsertRes) {
                                                if (CusTranInsertRes.length > 0) {
                                                    ...
                                                    var  UpdateTrnTbl = `update NPSS_CAMT_ACC_CRDR_NTFN   set  status='${success_status}',process_status='${success_process_status}',MODIFIED_BY = '${params.CREATED_BY}',MODIFIED_DATE = '${reqDateFormatter.GetTenantCurrentDateTime(headers, objSessionLogInfo)}',MODIFIED_BY_NAME ='${params.CREATED_BY_NAME}',PRCT_ID ='${PRCT_ID}', MODIFIED_CLIENTIP = '${objSessionLogInfo.CLIENTIP}', MODIFIED_TZ = '${objSessionLogInfo.CLIENTTZ}', MODIFIED_TZ_OFFSET = '${objSessionLogInfo.CLIENTTZ_OFFSET}', MODIFIED_BY_SESSIONID = '${objSessionLogInfo.SESSION_ID}', MODIFIED_DATE_UTC = '${reqDateFormatter.GetCurrentDateInUTC(headers, objSessionLogInfo)}'  where ncacn_id in ${TempTranID} `
                                                    ...
                                                    ExecuteQuery(UpdateTrnTbl, function (uptranresult) {
                                                        if (uptranresult == 'SUCCESS') {
                                                            ...
                                                            objresponse.status = 'Error in NPSS_CAMT_ACC_CRDR_NTFN Table Update';
                                                            sendResponse(null, objresponse)
