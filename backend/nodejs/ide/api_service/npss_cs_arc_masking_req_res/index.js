...
*/
var serviceName = 'NPSS (CS) Arc Masking Request Response';
            ...
            try {
                ...
                if (Gettrndetails.length > 0) {
                    ...
                    if (Gettrndetails[0].type == 'JSON') {
                        dataType = 'JSON'
                        Takedata = `select fn_pcidss_decrypt(request_data_json,$PCIDSS_KEY ) as request_json,fn_pcidss_decrypt(response_data_json,$PCIDSS_KEY) as response_json from arc_ad_gss_tran.npss_trn_process_log where npsstpl_id = '${params.npsstpl_id}'`
                    } else {
                        dataType = 'XML'
                        Takedata = `select fn_pcidss_decrypt(message_data,$PCIDSS_KEY ) as request_json from arc_ad_gss_tran.npss_trn_req_resp_dtls where npsstrrd_id = '${params.npsstrrd_id}'`
                    }
                    ...
                    ExecuteQuery1(Takedata, async function (arrdata) {
                        try {
                            if (arrdata.length > 0) {
                                ...
                                let Chckmskeli = `select param_category,param_detail from arc_ad_gss_tran.core_nc_system_setup where param_category = 'PROCESS_LOG_MASKING' and param_code = 'ROLE_ID' and param_detail = '${params.roleId}'`
                    ...
                    if (curntBtnName != "VIEW REQ AND RES") {
                        findData = `select fn_pcidss_decrypt(message_data,$PCIDSS_KEY ) as request_json from arc_ad_gss_tran.npss_trn_req_resp_dtls where npsstrrd_id = '${params.npsstrrd_id}'`

                    } else {
                        findData = `select fn_pcidss_decrypt(request_data_json,$PCIDSS_KEY ) as request_json,fn_pcidss_decrypt(response_data_json,$PCIDSS_KEY) as response_json from arc_ad_gss_tran.npss_trn_process_log where npsstpl_id = '${params.npsstpl_id}'`
                    }
