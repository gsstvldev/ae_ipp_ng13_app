...
app.post('/', function(appRequest, appResponse, next) {
    ...
    /*  Created By :   Daseen
        ...
        Modified Date : 03/05/2024 --suresh
        Reason for : 
        Modified By :  (NPSS WI - 3987)
        Modified Date : 31/07/2024 --Subramanian
        Reason for : Elp branch merged code with master
        * 
        */
                    ...
                    try {
                        ...
                        ExecuteQuery1(take_status, async function (arrSts) {
                            if (arrSts.length) {
                                ...
                                let arrNpssTrnReusult = await ExeQuery1(TakeStsPsts)
                                let arrUrlValue = await ExeQuery1(TakeApiurl), apicalRes
                            ...
                            return new Promise((resolve, reject) => {
                                try {
                                    ...
                                    var options = {
                                        ...
                                        method: 'POST',
                                        json: {
                                            "hdr_created_date": (arrresult.status.includes('IP_BCT_INVEST_INITIATED') && arrresult.process_type == 'IP') ? arrresult.hdr_created_date : arrresult.fx_resv_text1 || '',
                                            "cr_sort_code": arrresult.cr_sort_code,
                                            "payment_endtoend_id": arrresult.payment_endtoend_id,
                                            "uetr": arrresult.uetr,
                                            "tran_ref_id": arrresult.tran_ref_id,
                                            "payment_processing_method": arrresult.payment_processing_method,
                                            "process_type": arrresult.process_type || ''
                                        },
                                        headers: {
                                            'content-type': 'application/json'
                                        }
                                    };
                                    if (arrresult.process_type == 'IP') {
