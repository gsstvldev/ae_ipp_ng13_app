                            ...
                            if (arrSts.length) {
                                ...
                                ExecuteQuery1(Takedata, async function (arrprocesslog) {
                                    if (arrprocesslog.length > 0) {
                                        ...
                                        let Takerefno = `select npsstpl_id,processing_system,process_ref_no from npss_trn_process_log where status='OP_P2B_FUND_RESERVED'and process_status='RCTInProcess' and process_name='Fund Reserve INAU Posting' and uetr = '${arrprocesslog[0].uetr}'`

                                        ExecuteQuery1(Takerefno, async function (arrRefno) {
                                            if (arrRefno.length > 0) {
                                                var updateTable = await UpdateTran(arrRefno[0].npsstpl_id, PRCT_ID)
                                                var arrCusTranInst = []
                                                ...
                                                objresponse.status = 'FAILURE'
                                                objresponse.data = "No data found in npss trn process log table for FUND RESERV"
                                                sendResponse(null, objresponse)
                                            }
                                        })
