...
app.post('/', function(appRequest, appResponse, next) {
    ...
    */
    ...
    reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
	 ...
	 clrsysref,BENEFICIARY_BANK,End_to_End_ID,

	 T24_FT_REFERENCE_NUMBER,process_type,API_Success_Failure,Purpose_codes,channel_refno,

	 account_number,COALESCE(Split_part(report_icon_data,'base64',2),'') report_icon_data
...
FROM 
	...
	NT.dbtr_acct_name AS Debtor_Name,
	 ...
	 nt.intrbk_sttlm_amnt AS TRANSACTION_AMOUNT_RANGE,
	  ...
	  case when nt.process_group='MANUAL' then nt.instruction_id
	 ...
	 case when nt.process_group='MANUAL' then nt.category_purpose_prty
	...
	hdr_created_date AS CREATEDDATE,
	 ...
	 nt.VALUE_DATE,
		...
		case when ntpl.process_name = 'OP Debit Posting' and nt.process_group='P2P' then ntpl.process_ref_no

		when ntpl.process_name = 'Fund AUTH Posting' and nt.process_group='P2B' then ntpl.process_ref_no

		when ntpl.process_name='Manual Fund AUTH Posting' and nt.process_group='MANUAL' then ntpl.process_ref_no
...
FROM ${schema}.npss_transactions nt	
	  ...
	  left  join core_nc_cbs_accounts cnca  on nt.dbtr_iban = cnca.alternate_account_id
	  ...
	  LEFT JOIN core_member_banks cmb ON cmb.bic_code = nt.cr_sort_code AND cmb.NEED_SYNC = 'Y'
			...
			)v where process_type = 'OP'   ${cond_params}`
                            }
                            else {
                                Taketran = ` select
	distinct uetr,
