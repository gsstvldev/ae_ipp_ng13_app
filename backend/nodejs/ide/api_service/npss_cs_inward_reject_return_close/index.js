...
*/
    ...
    reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
        try {
            ...
            // Log Viewer Setup
            objSessionLogInfo.HANDLER_CODE = 'NPSS Inward Rejected Returns Close Logic';
            objSessionLogInfo.ACTION = 'ACTION';
            objSessionLogInfo.PROCESS = 'NPSS Inward Rejected Returns Close Logic';
            // Get DB Connection 
            reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                mTranConn = pSession; //  assign connection 
                //get prct id                              
                try {
                    var take_status = `Select success_process_status,success_status from core_nc_workflow_setup where rule_code='${params.rule_code}' and eligible_status='${params.eligible_status}' and and eligible_process_status='${params.eligible_process_status}'`;
                    ExecuteQuery1(take_status, function (arrSts) {
                        if (arrSts.length) {
                            ...
                            if (params.Tran_id) {
                                ...
                                ExecuteQuery1(uptTrnqry, function (trnRes) {
                                    if (trnRes == "SUCCESS") {
