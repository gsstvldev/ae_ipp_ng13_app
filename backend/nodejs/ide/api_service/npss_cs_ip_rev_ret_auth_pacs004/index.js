                   ...
                   Reason for : Adding Prepaid and Credit Card implementation 22/04/2023
                    Reason for :Update query changes 25/04/2023
                     ...
                     Reason for:Adding dbtr_iban,dbtr_acct_no  in api on 16/11/2023 by daseen
                       ...
                       Reason for : Elpaso Api posting payload changes included account no on 27/04/2024 by  Subramanian wi:3772
                       Reason for : for checking prepaid or credit iban taken from core_nc_system_setup on 30/04/2024 by  Subramanian wi:3772
                       Reason for  : for prepaid api cardidendifier_value taken from cdtr_iban for the key of cr_accnt_identification 2/4/2024 by renga WI :3771
                       Reason : ELP to Master CODE MERGED ON 11-07-2024 by Daseen
      */
      ...
      reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
        try {
          ...
          reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                          ...
                          ExecuteQuery1(takereturncode, async function (arrreturncode) {
                            if (arrreturncode.length > 0) {
                              ...
                              if (params.roleId == '708' || params.roleId == 708 || params.roleId == '738' || params.roleId == 738) { //for checking prepaid or credit only for checker
                                ...
                                apicalls = await checkprepaidorcredit(arrprocesslog, arrreturncode, params.screenName, lclinstrm, take_return_url, final_status, final_process_status, PRCT_ID, await getIbanParams(selIbanParamQry))
                              } else {
                                apicalls = 0
                              }
            ...
            function checkprepaidorcredit(arrprocesslog, arrreturncode, screenName, lclinstrm, take_return_url, final_status, final_process_status, PRCT_ID, arrIbanParamDetails) { //for checking prepid or credit card
              return new Promise((resolve, reject) => {
                let apitype = {}
            ...
            function CallPrepaidEplapsoApi(arrprocesslog, arrreturncode, screenName, lclinstrm, extIdentValue, reversalNo, apitype, Amount) {
              return new Promise(async (resolve, reject) => {
                try {
                  let Ac_number_Qry = `select * from npss_trn_process_log  where uetr='${arrprocesslog[0].uetr}' and process_name='Prepaid Card Validation' `
                  ...
                  ExecuteQuery1(Apiurl, function (arrurl) {
                    if (arrurl.length > 0) {
                      ExecuteQuery1(Ac_number_Qry, function (arrprcslog) {
                        try {
                          if (arrprcslog.length > 0) {
                            ...
                            var options = {
                              ...
                              json: {
                                "payload": {
                                  ...
                                  "elpaso_account_number": arrprcslog[0].account_number || '',  //key value need to be changed later
  
                                  "cr_acct_identification": (apitype.card_id_value ? arrprocesslog[0].cdtr_iban : arrprocesslog[0].cr_acct_identification) || '',//key value change for prepaid api
                                  "cr_acct_id_code": arrprocesslog[0].cr_acct_id_code || '',
                                  "message_data": arrprocesslog[0].message_data || '',
                            ...
                            objresponse.status = "FAILURE"
                            objresponse.errdata = `Account number for ${arrprcslog[0].uetr} not found`
                            sendResponse(null, objresponse)
                          }
                        } catch (error) {
                          reqInstanceHelper.PrintError(serviceName, objSessionLogInfo, "IDE_SERVICE_004", "ERROR IN API CALL FUNCTION", error);
