        ...
        reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
            try {
                ...
                reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                    ...
                    function fn_doapicall(url, arrprocesslog, lclinstrm, amount, reverandRefno, Objfiledata, Getdata, extend_retry_value, takedealRefno, callbackapi) {
                        try {
                        ...
                        return new Promise((resolve, reject) => {
                            if (reverseAcinfparam.currency == '' || reverseAcinfparam.currency == null) {
                                ...
                                if (reverseAcinfparam.currency != 'AED') {
                                    var TakeIpuetr = `select additional_info from npss_trn_process_log where process_name = 'Initiate Dispute Tran' and uetr = '${arrprocesslog[0].uetr}'`
                                    ExecuteQuery1(TakeIpuetr, function (arrIpuetr) {
                                        if (arrIpuetr.length > 0) {
                                            if (arrIpuetr[0].additional_info != null) {
                                                var Takedelrefno = `select process_ref_no from npss_trn_process_log where status = 'IP_RCT_REV_DEAL_RECEIVED' and process_name = 'Get Deal' and uetr = '${arrIpuetr[0].additional_info}'`
                                                ...
                                                objresponse.status = "UETR entry is missing"
                                                sendResponse(null, objresponse)
                                            }

                                        } else {
                                            objresponse.status = "Tran Process log dealrefno entry is missing"
                                            sendResponse(null, objresponse)
                                        }
