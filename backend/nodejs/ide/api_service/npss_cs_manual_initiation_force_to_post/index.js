    ...
    reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
       try {
          ...
          reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                                           ...
                                           ExecuteQuery1(take_api_url, async function (arrurl) {
                                              if (arrurl.length) {
                                                 ...
                                                 if (apicalls == 0) {
                                                    ...
                                                    dealrefno = await GetRefno(arrprocesslog,reverseAcinfparam)
                                                 } else {
                                                    TakegmMargin = {}
                                                 }
                                                 ...
                                                 if (InsertTable.length > 0) {
                                                    var callapi = async () => {
                                                       var apistatus = await checkapiCalls(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, TakegmMargin, apicalls,dealrefno)
             ...
             function fn_doapicall(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, TakegmMargin,dealrefno, callbackapi) {
                try {
             ...
             function checkapiCalls(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, TakegmMargin, apicalls,dealrefno) {
                return new Promise((resolve, reject) => {
                   // reserve fund
                   if (apicalls == 0) {
                      fn_doapicall(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, TakegmMargin,dealrefno, function (result) {
