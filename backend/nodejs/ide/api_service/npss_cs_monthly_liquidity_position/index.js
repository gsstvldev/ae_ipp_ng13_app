...
app.post('/', function(appRequest, appResponse, next) {
    ...
    */
    var serviceName = 'NPSS (CS) Monthly Liquidity';
    ...
    reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
        try {
            ...
            // Log Viewer Setup
            objSessionLogInfo.HANDLER_CODE = 'NPSS (CS) Monthly Liquidity';
            objSessionLogInfo.ACTION = 'ACTION';
            objSessionLogInfo.PROCESS = 'NPSS (CS) Monthly Liquidity';
            // Get DB Connection 
                        ...
                        function apiCall(arrTran, arrUrl) {
                            return new Promise((resolve, reject) => {
                                ...
                                try {
                                    ...
                                    var dt = moment(arrTran[0].datetime).format("YYYY-MM")
                                    var apiURL =
                                        apiURL = arrUrl[0].param_detail +'/monthlyLiquidityPosition?currency=' + arrTran[0].currency + '&accountHolderBic=' + arrTran[0].accountholderbic + '&accountNumber=' + arrTran[0].accountnumber + '&dateTime=' + dt;
                                    var options = {
                                        url: apiURL,
                                        timeout: 99999999,
                                        method: 'GET',
