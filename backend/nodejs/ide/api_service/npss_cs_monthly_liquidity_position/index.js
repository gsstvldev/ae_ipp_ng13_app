...
app.post('/', function(appRequest, appResponse, next) {
    ...
    Modified By : 
    Modified Date : 27/02/2023 6:50pm
    }
    */
    var serviceName = 'NPSS (CS) Monthly Liquidity Position';
    ...
    reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
        try {
            ...
            // Log Viewer Setup
            objSessionLogInfo.HANDLER_CODE = 'NPSS (CS) Monthly Liquidity Position';
            objSessionLogInfo.ACTION = 'ACTION';
            objSessionLogInfo.PROCESS = 'NPSS (CS) Monthly Liquidity Position';
            // Get DB Connection 
            reqTranDBInstance.GetTranDBConn(headers, false, async function (pSession) {
                ...
                reqAuditLog.GetProcessToken(pSession, objLogInfo, async function prct(error, prct_id) {
                    try {
                        ...
                        var PRCT_ID = prct_id
                        var TakeFinalStatus = `select success_process_status,success_status from core_nc_workflow_setup where rule_code = 'CORE_API_MNTH_LIQ_POSITION' and  eligible_status = '${params.eligible_status}' and eligible_process_status = '${params.eligible_process_status}'`
                            ...
                            if (arrStatus.length > 0) {
                                ...
                                ExecuteQuery1(Taketran, function (arrTran) {
                                    if (arrTran.length > 0) {
                                        ...
                                        objresponse.status = 'No data found  in npss core api table';
                                        sendResponse(null, objresponse);
                                    }
                        ...
                        function apiCall(arrTranobj, arrUrl) {
                            return new Promise((resolve, reject) => {
                                ...
                                try {
                                    ...
                                    var dt = moment(arrTranobj.datetime).format("YYYY-MM")
                                    var apiURL =
                                        apiURL = arrUrl[0].param_detail +'/monthlyLiquidityPosition?currency=' + arrTranobj.currency + '&accountHolderBic=' + arrTranobj.accountholderbic + '&accountNumber=' + arrTranobj.accountnumber + '&dateTime=' + dt;
                                    var options = {
                                        url: apiURL,
                                        timeout: 99999999,
                                        method: 'GET',
