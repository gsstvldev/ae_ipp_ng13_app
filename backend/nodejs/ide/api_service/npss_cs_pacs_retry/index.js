            ...
            reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                ...
                function PrepareApidata(arrdata, org_status, process_name, ProcessStatus, CommonData) {
                    return new Promise(async (resolve, reject) => {
                        ...
                        let process_detail
                        if (org_status == 'OP_P2P_FILE_PLACED' || org_status == 'OP_AC_FILE_PLACED' || org_status == 'OP_P2B_PAYMENT_INITIATED' || org_status == 'OP_P2P_PAYMENT_INITIATED' || org_status == 'OP_MAN_FILE_PLACED') {
                            process_detail = 'Pacs.008 Real Time Credit Transfer'
                        }

                        if (org_status == 'OP_P2B_FILE_PLACED') {
                            process_detail = 'Pacs.008 Real Time Credit Transfer P2B'
                        }

                        if (org_status == 'OP_RCT_REVERSAL_PLACED'){
                            process_detail = 'pacs.007 Payment Reversal Request'
                        }
                            
                        if (org_status == 'IP_RCT_RETURNED' || org_status == 'IP_RCT_RR_RETURNED'){
                            process_detail = 'Pacs.004 Payment Return'
                        }
                            
                        if (org_status == 'IP_RCT_REVREQ_REJ_REPLIED'){
                            process_detail = 'Pacs.002 Payment Reversal Request Reject Status'
                        }
                           
                        if (org_status == 'IP_RCT_CC_POSTING_READY' || org_status == 'IP_RCT_PC_POSTING_READY' || org_status == 'IP_RCT_POSTING_READY' || org_status == 'IP_RCT_CC_STATUS_REPLIED' || org_status == 'IP_RCT_PC_STATUS_REPLIED' || org_status == 'IP_RCT_STATUS_REPLIED'){
                            process_detail = 'Pacs.002 (Account Validation Response)'
                        }
                            
                        if (org_status == 'OP_AC_PAYMENT_INVESTIGATED' || org_status == 'OP_P2P_INVESTIGATED' || org_status == 'OP_P2B_INVESTIGATED' || org_status == 'OP_MAN_INVESTIGATED' || org_status == 'IP_RCT_INVESTIGATION_INITIATED'){
                            process_detail = 'Investigation_pacs028'
                        }
                        ...
                        body = {
                            ...
                            "data": {
                                "payload": {
                                    ...
                                    "process_detail": process_detail ? process_detail : '',
                                    "retry_count": 1
                                }
                            }
                        }
