...
reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
    try {
        ...
        // Log Viewer Setup
        objSessionLogInfo.HANDLER_CODE = 'NPSS (CS) Payment List';
        objSessionLogInfo.ACTION = 'ACTION';
        objSessionLogInfo.PROCESS = 'NPSS (CS) Payment List';
        // Get DB Connection 
        reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
            ...
            reqAuditLog.GetProcessToken(pSession, objLogInfo, function prct(error, prct_id) {
                try {
                    var PRCT_ID = prct_id
                    var TakeFinalStatus = `select success_process_status,success_status from core_nc_workflow_setup where rule_code = 'CORE_API_PAYMENT_LIST' and  eligible_status = '${params.eligible_status}' and eligible_process_status = '${params.eligible_process_status}'`
                    ...
                    function apiCall(arrTranobj, arrUrl) {
                        return new Promise((resolve, reject) => {
                            var Fromdate = moment(arrTranobj.fromdate).format('YYYY-MM-DD')+'T00:00:00.000'
                            var todate = moment(arrTranobj.todate).format('YYYY-MM-DD')+'T00:00:00.000'
                            try {

                                var request = require('request');
                                
                                var apiURL =    
                                apiURL = arrUrl[0].param_detail+'/paymentList?senderBic='+arrTranobj.senderbic+'&dateTo='+todate+'&dateFrom='+Fromdate+'&datasource='+arrTranobj.datasource
                                var options = {
                                    url: apiURL,
                                    timeout: 99999999,
                                    method: 'GET',
