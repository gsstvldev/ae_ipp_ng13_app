...
app.post('/', function(appRequest, appResponse, next) {
    ...
    try {
      ...
      Siva Harish-- Adding returnid 24/06/2023 in T24 inward return Nostro api 
         ...
         modified by - renga - only credit api for uab process 9-04-24
                ...
                reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                    ...
                    reqAuditLog.GetProcessToken(pSession, objLogInfo, function prct(error, prct_id) {
                        try {
                            ...
                            var Takeapiurl = `Select param_category,param_code,param_detail from core_nc_system_setup where param_category='INWARD_RETRY_REPOST' and param_code='URL' and need_sync = 'Y'`
                                    ...
                                    if (params.Roleid == '705' || params.Roleid == 705 || params.eligible_status.startsWith('IP_BCT')) {
                                        ...
                                        ExecuteQuery1(take_api_params, async function (arrTranparams) {
                                            if (arrTranparams.length > 0) {
                                                ExecuteQuery1(Takeapiurl, async function (arrurl) {
                                                    if (arrurl.length > 0) {
                                                        var Apicalls

                                                        let ext_ident_value = ''
                                                        //ext_ident_value = await GetRetrycount(arrTranparams[0].uetr)
                                                         if (params.eligible_status == 'IP_RCT_CC_T24_POSTING_RETRY' || params.eligible_status == 'IP_RCT_CC_POSTING_RETRY') { //credit                                     
                                            ...
                                            ExecuteQuery1(TakenpsstrdRefno, function (arrnpssRefno) {
                                                if (arrnpssRefno.length > 0) {
                                                    ...
                                                    if (params.eligible_status == 'IP_RCT_CC_T24_POSTING_RETRY') {
                                                        options.json.payload.NOSTRO_POOL_ACC = 'Y'
                                                    }
