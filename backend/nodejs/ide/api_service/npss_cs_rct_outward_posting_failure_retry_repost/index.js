...
app.post('/', function(appRequest, appResponse, next) {
    ...
    try {
        ...
        Modified For : Kafka url only for checker role  on 15-04-2024 -- renga
        ...
        reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
            try {
                ...
                reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                                    ...
                                    if (params.Roleid == '705' || params.Roleid == 705) {
                                        ...
                                        ExecuteQuery1(take_api_params, function (arrTranparams) {
                                            if (arrTranparams.length > 0) {
                                                ...
                                                objresponse.status = 'FAILURE'
                                                objresponse.errdata = 'No data in Tran Table'
                                                sendResponse(null, objresponse)
                                            }
                                        })
                                        ...
                                        ExecuteQuery1(take_api_params, function (arrTranparams) {
                                            if (arrTranparams.length > 0) {
                                                ...
                                                var Apicalls
                                                if (params.eligible_status == 'OP_AC_REV_POSTING_RETRY' || params.eligible_status == 'OP_AC_RET_POSTING_RETRY' || params.eligible_status == 'OP_P2P_REV_POSTING_RETRY' || params.eligible_status == 'OR_P2P_POSTING_RETRY' || params.eligible_status == 'OP_P2P_DBT_POSTING_RETRY' || params.eligible_status == 'OR_P2B_POSTING_RETRY' || params.eligible_status == 'OP_P2B_FUND_AUTH_RETRY' || params.eligible_status == 'OP_P2B_FUND_UNFR_RETRY' || params.eligible_status == 'OP_P2B_REV_POSTING_RETRY' ) { //ORR
                                                    ExecuteQuery1(Takekafkaurl, async function (arrurl) {
                                                        if (arrurl.length > 0) {
                                                            Apicalls = await apicall(arrTranparams, failcountobj, failcount, arrurl, params.eligible_status)
                                                            if (Apicalls == 'SUCCESS') {
                                                                InsertProcess(arrTranparams, final_process_status, final_status, PRCT_ID)
                                                            } else {
                    ...
                    function apicall(arrTranparams, failcountobj, failcount, arrurl, status) {
                        return new Promise((resolve, reject) => {
                            reqAsync.forEachOfSeries(arrTranparams, function (arrTranparamsObj, i, nextobjctfunc) {
                                var takeprcsref_no = `select process_ref_no from npss_trn_process_log  where uetr ='${arrTranparamsObj.uetr}' and process_name ='Outward Debit Posting' and status='OP_P2P_POSTING_SUCCESS'  `
                                ExecuteQuery1(takeprcsref_no, function (arrprcsrefno) {
                                    let process_type = ''
                                    let process_ref_no = ''
                                    if (arrprcsrefno.length > 0 && status != 'OP_P2P_DBT_POSTING_RETRY') {
                                        process_ref_no = arrprcsrefno[0].process_ref_no
                                    } else {
                                        process_ref_no = ''
                                    }
                                    if (status == 'OP_AC_REV_POSTING_RETRY') {
                                        process_type = 'ORR'

                                    }
                                    else if (status == 'OP_AC_RET_POSTING_RETRY') {
                                        process_type = 'OR'

                                    }
                                    else if (status == 'OP_P2P_REV_POSTING_RETRY') {
                                        process_type = 'P2PORR'

                                    }
                                    else if (status == 'OR_P2P_POSTING_RETRY') {
                                        process_type = 'P2POR'

                                    }
                                    else if (status == 'OP_P2P_DBT_POSTING_RETRY') {
                                        process_type = 'OP'

                                    } else if (status == 'OP_P2B_REV_POSTING_RETRY') {
                                        process_type = 'P2BORR'
                                    } else if (status == 'OP_P2B_FUND_UNFR_RETRY') {
                                        process_type = 'UNFR'
                                    } else if (status == 'OP_P2B_FUND_AUTH_RETRY') {
                                        process_type = 'AUTH'
                                    } else if (status == 'OR_P2B_POSTING_RETRY') {
                                        process_type = 'P2BOR'
                                    }
                                            ...
                                            json: {
                                                ...
                                                data: {
                                                    "payload": {
                                                        ...
                                                        "api_request_id": '',
                                                        "account_currency": arrTranparamsObj.intrbk_sttlm_cur || ''
                                                    }
                                                }
                                            },
                                            headers: {
