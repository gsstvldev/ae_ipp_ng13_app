                ...
                reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                    ...
                    reqAuditLog.GetProcessToken(pSession, objLogInfo, function prct(error, prct_id) {
                        try {
                            ...
                            var take_api_url = `Select param_category,param_code,param_detail from core_nc_system_setup where param_category='NPSS_REVERSE_USABLE_BALANCE' and param_code='URL' and need_sync = 'Y'`;
                            var take_api_params = `select fn_pcidss_decrypt(ns.cr_acct_identification,$PCIDSS_KEY ) as cr_acct_identification,ns.remittance_info,ns.cr_acct_id_code,ns.hdr_msg_id,ns.hdr_created_date,ns.hdr_total_records,ns.hdr_total_amount,ns.hdr_settlement_date,ns.hdr_settlement_method, ns.hdr_clearing_system,ns.dr_sort_code,ns.cr_sort_code,ns.category_purpose,ns.category_purpose_prty,ns.ext_purpose_code,ns.ext_purpose_prty,ns.intrbk_sttlm_amnt, ns.uetr,ns.intrbk_sttlm_cur,ns.dbtr_iban,ns.cdtr_iban,ns.dbtr_acct_name,ns.cdtr_acct_name,ns.payment_endtoend_id,ns.charge_bearer ,fn_pcidss_decrypt(ns.message_data,$PCIDSS_KEY ) as message_data,ns.reversal_amount, ns.process_type,ns.status,ns.process_status,ns.tran_ref_id txid,ns.tran_ref_id, value_date,ext_org_id_code,process_type,clrsysref,accp_date_time as accp_dt_tm from ${schema}.npss_transactions ns where npsst_id = '${params.Tran_Id}'`;
                            ...
                            ExecuteQuery1(take_api_params, async function (arrprocesslog) {
                                if (arrprocesslog.length) {
                                    ...
                                    if (arrprocesslog[0].cdtr_iban == null || arrprocesslog[0].cdtr_iban == '') {
                                        ...
                                        let IBAN, type = ''
                                        var ChckPrePaidIban = `select * from core_nc_system_setup where param_category='ELPASO_PREPAID_CARD' and param_Code='BRANCH_CODES'`
                                        let PrepaidIban = await PrePaidIbanQry(ChckPrePaidIban)
                                        ...
                                        if (type == 'cdtr' || type == 'dbtr') {
                                            ...
                                            if (result.toUpperCase() == 'IBAN_MISMATCH') {
                                                var TakeAcctInf = `select Alternate_Account_Type,currency,account_number,alternate_account_id,inactive_marker,company_code,curr_rate_segment,customer_id,account_officer from core_nc_cbs_accounts where alternate_account_id = '${IBAN}'`
                    ...
                    //CHECK PARAM_DETAIL MATCH WITH IBAN FOR CREDIT OR DEBIT
                    function CheckIban(IBAN, PrepaidIban, type) {
                        return new Promise((resolve, reject) => {
                            let Iban_S = IBAN.slice(-16)
                            let FrmIban = Iban_S.substring(0, 3)
                            //if (FrmIban == '564' || FrmIban == 564) {
                            if (PrepaidIban.filter((val) => val.param_detail == FrmIban).length > 0) {
                                if (type == 'cdtr') {
                                    objresponse.status = 'Rectibi';
                                    objresponse.data = 'Rectibi CDTR Tran';
                                    sendResponse(null, objresponse);
                                }
                                else {
                                    objresponse.status = 'Rectibi';
                    ...
                    function PrePaidIbanQry(qry) {
                        return new Promise((resolve, reject) => {
                            ExecuteQuery1(qry, function (arresult) {
                                resolve(arresult)
                            })
                        })
                    }
