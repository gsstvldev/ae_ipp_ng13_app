               ...
               Reason for :Changing INSERT payload and query 8/2/2023
                Reason for : Removing pacs002 api call for bot fh and fab 16/03/2023
                 ...
                 Reason for : Handling prepaid and credit 21/04/2023
                  ...
                  Reason for : Adding new insert process log for prepaid and credit wi:3756 18/04/2024
                ...
                reqAuditLog.GetProcessToken(pSession, objLogInfo, function prct(error, prct_id) {
                    if (error) {
                        ...
                        try {
                            ...
                            function UpdateTranDB(success_process_status, success_status, arrdata, PRCT_ID, arrrule, arrdata) {
                                ...
                                ExecuteQuery(updtranqry, async function (uptranresult) {
                                    try {
                                        ...
                                        reqInstanceHelper.PrintError(serviceName, objSessionLogInfo, error)
                                        sendResponse(null, { status: "Failure", data: error })
                                    }
                                })
                            }
                            function doInsertInProcessLog(objCusTranInst) {
                                return new Promise((resolve, reject) => {
                                    try {
                                        let arrCusTranInst = []
                                        arrCusTranInst.push(objCusTranInst)
                                        _BulkInsertProcessItem(arrCusTranInst, 'NPSS_TRN_PROCESS_LOG', function callbackInsert(CusTranInsertRes) {
                                            try {
                                                if (CusTranInsertRes.length > 0) {
                                                    ...
                                                    let failure_data = "Processlog insert failure"
                                                    reqInstanceHelper.PrintError(serviceName, objSessionLogInfo, error)
                                                    sendResponse(null, { status: "Failure", data: failure_data })
                            ...
                            function doFormParamsForPrcloginsert(arrdata, success_process_status, success_status, arrrule) {
                                return new Promise((resolve, reject) => {
                                    let objCusTranInst = {}
                                    try {
