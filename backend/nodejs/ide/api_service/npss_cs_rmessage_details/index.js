...
*/
var serviceName = 'NPSS (CS) Rmessage Details';
...
reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
    try {
        ...
        // Log Viewer Setup
        objSessionLogInfo.HANDLER_CODE = 'NPSS (CS) Rmessage Details';
        objSessionLogInfo.ACTION = 'ACTION';
        objSessionLogInfo.PROCESS = 'NPSS (CS) Rmessage Details';
        // Get DB Connection 
                                        ...
                                        if (arrUrl.length > 0) {
                                            ...
                                            _BulkInsertProcessItem(arrCoreApiInst, 'npss_core_api_process_log', function callbackInsert(CoreTranInsertRes) {
                                                if (CoreTranInsertRes.length > 0) {  
                                                    ...
                                                    objresponse.status = 'Error in log Table Insert';
                                                    sendResponse(null, objresponse)
                                                }
                                            })
                    ...
                    function apiCall(CoreTranInsertRes, arrUrl) {
                        return new Promise((resolve, reject) => {
                            ...
                            try {
                                ...
                                var dt = moment(params.datetime).format("YYYY-MM-DD")
                                var apiURL =

                                    apiURL = arrUrl[0].param_detail + '/rMessageDetail?ref-type=' + params.reftype + '&ref=' + params.ref + '&source=' + params.datasource + '&payment-date=' + dt + '&sender=' + params.senderbic
                                var options = {
                                    url: apiURL,
                                    timeout: 99999999,
                                    method: 'GET',
                                ...
                                request(options,  function (error, responseFromImagingService, responseBody) {
                                    try {
                                        if (error) {
                                            ...
                                            try {
                                                reqInstanceHelper.PrintInfo(serviceName, '------------API Response JSON-------' + JSON.parse(responseBody), objSessionLogInfo);
                                                resolve('SUCCESS');
                                            } catch (error) {
