        ...
        reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
          try {
            ...
            reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
              ...
              function checkAllValues(arrprocesslog, amount, reverseAcinfparam, extend_retry_value) {
                return new Promise((resolve, reject) => {
                  try {
                    //let err_msg = ''
                    ...
                    if (arrprocesslog[0].hdr_msg_id == undefined || arrprocesslog[0].hdr_msg_id == null) {
                      throw new Error('hdr msg id Value is Missing')
                    }
                    /* if (arrprocesslog[0].process_type == undefined || arrprocesslog[0].process_type == null || arrprocesslog[0].process_type == '') {
                      throw new Error('process Type Value is Missing')
                    } */
                    /*
                     if (arrprocesslog[0].hdr_total_records == undefined || arrprocesslog[0].hdr_total_records == null || arrprocesslog[0].hdr_total_records == '') {
                      throw new Error('hdr Total Records Value is Missing')
                    } */
                    ...
                    if (arrprocesslog[0].dbtr_iban == undefined || arrprocesslog[0].dbtr_iban == null) {
                      throw new Error('dbtr iban Value is Missing')
                    }
              ...
              function fn_doPrepaidapicall(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, extend_retry_value, callbackapi) {
                try {
                  ...
                  const checkValueFn = async () => {
                    null_value_status = await checkAllValues(arrprocesslog, amount, reverseAcinfparam, extend_retry_value)
                  }
                  checkValueFn()
