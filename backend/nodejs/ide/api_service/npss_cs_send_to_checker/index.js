    ...
    reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
        try {
            ...
            reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                                                            ...
                                                            if (checkForceTopost == 'Call_Reserve_Fund_Api') {
                                                                ...
                                                                ExecuteQuery1(take_api_url, async function (arrurl) {
                                                                    if (arrurl.length) {
                                                                        ...
                                                                        var callapi = async () => {
                                                                            var apistatus = await checkapiCalls(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, Objfiledata, TakegmMargin, apicalls)
                ...
                function fn_doapicall(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, Objfiledata, TakegmMargin, callbackapi) {
                    try {
                ...
                function checkapiCalls(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, Objfiledata, TakegmMargin, apicalls) {
                    return new Promise((resolve, reject) => {
                        // reserve fund
                        if (apicalls == 0) {
                            fn_doapicall(url, arrprocesslog, lclinstrm, amount, reverseAcinfparam, Objfiledata, TakegmMargin, function (result) {
