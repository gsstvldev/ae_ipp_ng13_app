                    ...
                    try {
                                    ...
                                    if (arrData.length > 0) {
                                        ...
                                        if (updatetrn == 'SUCCESS') {
                                            reqAsync.forEachOfSeries(arrData, function (arrDataobj, i, nextobjctfunc) {
                                                var checkcond = `select npsstrrd_refno, * from npss_trn_process_log where status in ${Secondstatus} and process_name in ${SecondprocessName} and npsstrrd_refno='${arrDataobj.npsstrrd_refno}' `
                                                ExecuteQuery1(checkcond, function (arrcheckcond) {
                                                    if (arrcheckcond.length == 0) {
                                                        ...
                                                        ExecuteQuery1(Takeurl, async function (arrurl) {
                                                            if (arrurl.length > 0) {
                                                                var apicallresult = await kafkaapi(arrDataobj, arrurl)
                                                                if (apicallresult == 'SUCCESS') {
                                                                    var insertlog = await ProcessInstData(arrDataobj, PRCT_ID)
                        ...
                        function ProcessInstData(arrfundauth, PRCT_ID) {
                            return new Promise((resolve, reject) => {
                                ...
                                objCusTranInst.PROCESSING_SYSTEM = 'Cinque';
                        ...
                        function kafkaapi(arrpayverobj, arrurl) {
                            var postrefno;
                            return new Promise((resolve, reject) => {
