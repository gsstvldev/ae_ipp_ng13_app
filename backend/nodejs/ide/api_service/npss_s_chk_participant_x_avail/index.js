...
app.post('/', function(appRequest, appResponse, next) {
    ...
    /*  Created By :   Siva Harish
         ...
         Reason for Changing select query 09/05/2023
         Reason for Changing prct_tokens insert based on eligible 05/08/2024
                                                               ...
                                                               //  var TakeOffset = `SELECT setup_json FROM clt_tran.TENANT_SETUP WHERE TENANT_ID ='${params.tenant_id}' AND CATEGORY='TIMEZONE'`
                                                                var TakeOffset = `SELECT setup_json FROM clt_tran.TENANT_SETUP WHERE TENANT_ID ='${params.tenant_id}' AND CATEGORY='TIMEZONE'`
                                                                 ExecuteQuery1(TakeOffset, async function (arroffset) {
                                                                     ...
                                                                     var takeparticipantList = `select * from core_nc_bank_part_avail where  from_date = '${currdate}' and cast (from_time as time) - INTERVAL '1 minutes' = '${PrepareDate}' and status = 'APPROVED' and need_sync = 'Y'`
