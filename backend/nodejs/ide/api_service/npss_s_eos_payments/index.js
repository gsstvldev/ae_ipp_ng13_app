...
app.post('/', function(appRequest, appResponse, next) {
  ...
  Reason : Removed duplicate entries on 07-02-2024 by Daseen
  Reason : nly for IP transaction on 13-02-2024 by Daseen
  Reason : Calling process token function only in eligible cases on 08-05-2024 by Subramanian
  }
  */
          ...
          reqTranDBInstance.GetTranDBConn(headers, false,async  function (pSession) {
            ...
            //  reqAuditLog.GetProcessToken(pSession, objLogInfo,async function prct(error, prct_id) {
                  ...
                  try {
                      ...
                      if (arrTran.length > 0) {
                          let trnId = arrTran.map((x) => x.npsst_id)
                           let dupRemvTrn=[]
                              ...
                              var updateRes = await updateEligibleTran('(' + dupRemvTrn.join(',') + ')',prct_id)
                      ...
                      function updateEligibleTran(contn,prct_id) {
                          return new Promise((resolve, reject) => {
