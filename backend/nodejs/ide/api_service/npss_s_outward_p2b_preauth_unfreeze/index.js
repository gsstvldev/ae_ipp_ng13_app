                                   ...
                                   if (arrpayver.length > 0) {
                                       reqAsync.forEachOfSeries(arrpayver, function (arrpayverobj, i, nextobjctfunc) {
                                           if (arrpayverobj.npsstrrd_refno) {
                                               ...
                                               var add_hrs = m.add(arrhour[0].param_value, 'days')

                                               var Formdate = moment(add_hrs).format('YYYY-MM-DD HH:mm:ss')
                                               takefundauth = `select npsstpl_id, * from npss_trn_process_log where status in ${status2} and process_name in ${process_name2} and npsstrrd_refno='${arrpayverobj.npsstrrd_refno}' and created_date_utc>='${Formdate} and additional_info  in ${additional_info2}'`
