            ...
            reqTranDBInstance.GetTranDBConn(headers, false, async function (pSession) {
                ...
                reqAuditLog.GetProcessToken(pSession, objLogInfo, async function prct(error, prct_id) {
                    try {
                                                        ...
                                                        ExecuteQuery1(Takeurl, function (arrurl) {
                                                            if (arrurl.length > 0) {
                                                                ...
                                                                ExecuteQuery1(taketran, async function (arrTran) {
                                                                    if (arrTran.length > 0) {
                                                                        var apicallresult = await kafkaapi(arrTran, arrDataobj, arrurl,TakerequestData)
                        ...
                        //kafka api call
                        function kafkaapi(arrTranparams, arrpayverobj, arrurl, process_name) {
                            var postrefno;
                            return new Promise((resolve, reject) => {
                                            ...
                                            if(arrtakeacctinfo.length > 0){
                                                ...
                                                ExecuteQuery1(seldetqry, function (arrselldet) {
                                                    if(arrselldet.length > 0){
                                                        var takereqjson = `select npsstpl_id,fn_pcidss_decrypt(request_data_json,$PCIDSS_KEY) as request_data_json,npsstrrd_refno,msg_id, additional_info as tran_type_code  from npss_trn_process_log where process_name in ${process_name}  and uetr='${arrpayverobj.uetr}'`
                                                                ...
                                                                if (arrtakereqjson[0].request_data_json != null) {
                                                                    ...
                                                                    ExecuteQuery1(takebiccode, function (arrtakebiccode) {
                                                                        if(arrtakebiccode.length > 0){
                                                                            ...
                                                                            reqInstanceHelper.PrintInfo(serviceName, '------------Resipient Bankcode not found for uetr-------' + arrpayverobj.uetr, objSessionLogInfo);
                                                                            resolve('FAILURE')
                                                                        }
                                                        ...
                                                        reqInstanceHelper.PrintInfo(serviceName, '------------Sell Margin not found for uetr-------' + arrpayverobj.uetr, objSessionLogInfo);
                                                        resolve('FAILURE')
                                                    }
                                                ...
                                                reqInstanceHelper.PrintInfo(serviceName, '------------Customer Information not found -------' + arrpayverobj.uetr, objSessionLogInfo);
                                                resolve('FAILURE')
                                            }
