                                           ...
                                           ExecuteQuery1(takeTrn, function (arrTrn) {
                                               if (arrTrn.length > 0) {
                                                   reqAsync.forEachOfSeries(arrTrn, function (arrTrnobj, i, nextobjctfunc) {
                                                       ...
                                                       var TakeLogandAcct=`select fn_pcidss_decrypt(cr_acct_identification,$PCIDSS_KEY) as cr_acct_identification, nt.created_Date as tran_created_date,nt.modified_date as tran_modified_date,nt.status
                                                       ...
                                                       as acct_status,ac.issuer_type_code as acct_issuer_type_code,ac.* from npss_trn_process_log l left join npss_transactions nt on l.uetr=nt.uetr left join core_nc_cbs_accounts ac on ac.alternate_account_id in(nt.cdtr_iban,cr_acct_identification) where l.uetr='${arrTrnobj.uetr}' order by npsstpl_id`
                                                       ExecuteQuery1(Takelog, function (arrlog) {
                                                           if (arrlog.length > 0) {
                                                            ExecuteQuery1(TakeLogandAcct,function(arrAllData)
                                                            {
                                                                ...
                                                                console.log(i)
                                                                   var TakeCometo = `select param_value from CORE_NS_PARAMS  where process_name = '${params.process_name}' and destination_system = '${arrTrnobj.department_code}' and param_name='COMM_TO' and need_sync = 'Y'`
                                                                               ExecuteQuery1(TakeCometo, function (arrCometo) {
                                                                                var Takecmcc = `select param_value from CORE_NS_PARAMS  where process_name = '${params.process_name}' and destination_system = '${arrTrnobj.department_code}' and param_name = 'COMM_CC' and need_sync = 'Y'`
                                                                                ExecuteQuery1(Takecmcc, function (arrcomcc) {
                                                                                    var Takebcc = `select param_value from CORE_NS_PARAMS  where process_name = '${params.process_name}' and destination_system = '${arrTrnobj.department_code}' and param_name = 'COMM_BCC' and need_sync = 'Y'`
                                                                                    ExecuteQuery1(Takebcc, function (arrbcc) {
                                                                                        var tkcomgp = `select param_value from CORE_NS_PARAMS  where process_name = '${params.process_name}' and destination_system = '${arrTrnobj.department_code}' and param_name = 'COMM_GROUP' and need_sync = 'Y'`
                                                                                        ExecuteQuery1(tkcomgp, function (arrcomgp) {  
                                                                                                var TakecomCat = `select param_value from CORE_NS_PARAMS  where process_name = '${params.process_name}' and destination_system = '${arrTrnobj.department_code}'  and param_name = 'COM_CATEGORY' and need_sync = 'Y'`
                                                                           ...
                                                                           TRAN_STATUS: arrAllData[i].tran_status?arrAllData[0].tran_status:"",
