        ...
        reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
            try {
                ...
                reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
                                            ...
                                            if (arrdata.length > 0) {
                                                let Callapis = async () => {
                                                    ...
                                                    if (arrdata[0].status == params.eligible_status) {
                                                        let UpdateEliTran = await UpdasEligible(arrdistuetrObj.uetr)
                                                        if(UpdateEliTran == 'SUCCESS'){
                                                                                        ...
                                                                                        if (error) {
                                                                                            ...
                                                                                            try {
                                                                                                if (responseBodyFromImagingService == 'SUCCESS') {
                                                                                                    ...
                                                                                                    ReUpdateEliTran = await ReUpdasNotEligible(arrdistuetrObj.uetr)
                                                         ...
                                                         ReUpdateEliTran = await ReUpdasNotEligible(arrdistuetrObj.uetr)
                                                            reqInstanceHelper.PrintInfo(serviceName, ".......................Problem in Eligible status update...............", +org_status + ".................UETR...........", +arrdistuetrObj.uetr, objSessionLogInfo);
                                                            nextuetrobjfun()  
                                                        }
                    ...
                    function UpdasEligible(uetr) {
                        return new Promise((resolve,reject)=>{
                            let UpdTran = `update npss_trn_process_log set status = 'CBUAE_NOT_AVAIL_TAKEN' where status = '${params.eligible_status}' and uetr = '${uetr}'`
                            ExecuteQuery(UpdTran, function (updTran) {
                                if(updTran == 'SUCCESS'){
                                    resolve('SUCCESS')
                                }else{
                                    resolve('FAILURE')
                    ...
                    function  ReUpdasNotEligible(uetr){
                        return new Promise((resolve,reject)=>{
                            let UpdTran = `update npss_transactions set status = '${params.eligible_status}' where status = 'CBUAE_NOT_AVAIL_TAKEN' and uetr = '${uetr}'`
                            ExecuteQuery(UpdTran, function (updTran) {
                                if(updTran == 'SUCCESS'){
                                    resolve('SUCCESS')
                                }else{
