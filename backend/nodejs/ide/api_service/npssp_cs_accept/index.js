...
reqLogInfo.AssignLogInfoDetail(appRequest, function (objLogInfo, objSessionInformation) {
    try {
        ...
        // Log Viewer Setup
        objSessionLogInfo.HANDLER_CODE = 'NPSSP  Accept';
        objSessionLogInfo.ACTION = 'ACTION';
        objSessionLogInfo.PROCESS = 'NPSSP Accept';
        // Get DB Connection 
            ...
            reqAuditLog.GetProcessToken(pSession, objLogInfo, function prct(error, prct_id) {
                if (error) {
                    ...
                    try {
                        ...
                        try {
                                                   //newly modified 
                            var TakeCdtrDtls
                            var TkDrdetails = ` select td.processdata from transactions a inner join TransactionDetails td on a.trnid =  td.trnid  where msgid = '${params.unique_Id}'`
                            ExecuteQuery1(TkDrdetails, function (arrDrcustDtls) {
                                if (arrDrcustDtls.length) {
                                    ...
                                    fn_CallNestApi(xmlfile, function (result) {
                                        if (result.outcome == '000') {
                                           /// objresponse.status = 'SUCCESS';
                                           // sendResponse(null, objresponse)
                                           var Tkow = ` select td.processdata from transactions a inner join TransactionDetails td on a.trnid =  td.trnid  where msgid = '${params.unique_Id}'`
                                           ExecuteQuery1(Tkow, function (arrowDtls) {
                                            if (arrowDtls.length) {
                                                var buff = arrowDtls[0].processdata
                                                var owxmlfile = buff.toString()
                                               // var processdata = xmlfile.replace('<MsgId>'+params.Id+'</MsgId>','<MsgId>'+params.unique_id+'</MsgId>')
                                                var processowdata = owxmlfile.replace('<OrgnlMsgId>'+params.unique_Id+'</OrgnlMsgId>','<OrgnlMsgId>'+params.Id+'</OrgnlMsgId>')
                                                console.log(processowdata);
                                                fn_CallNestApi(processowdata, function (result) {
                                                    if (result.outcome == '000') {
                                                        objresponse.status = 'SUCCESS';
                                                        sendResponse(null, objresponse)
                                                    } else {
                                            ...
                                            objresponse.status = result.errorMsg;
                                            sendResponse(null, objresponse)
                                        }
                                    })
                ...
                function fn_CallNestApi(apiparam, callbackapi) {
                    try {
                        ...
                        var options = {
                            url: 'http://192.168.2.40:3000/simulation/sct-initiation/pacs002-sender-inward-payment-status-report',
                        ...
                        request(options, function (error, responseFromImagingService, responseBodyFromImagingService) {
                            if (error) {
                                ...
                                reqInstanceHelper.PrintInfo(serviceName, '------------IPSP Accept-------' + responseBodyFromImagingService, objSessionLogInfo);
