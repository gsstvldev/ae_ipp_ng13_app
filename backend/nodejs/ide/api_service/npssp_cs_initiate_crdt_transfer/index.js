...
*/
var serviceName = 'NPSSP Intiate Credit Transfer';
    ...
    try {
        ...
        reqTranDBInstance.GetTranDBConn(headers, false, function (pSession) {
            ...
            reqAuditLog.GetProcessToken(pSession, objLogInfo, function prct(error, prct_id) {
                                                    ...
                                                    if (arrCdtrDtls.length) {
                                                        ...
                                                        ExecuteQuery1(TakeCrBnkCode, function (arrCrbnkcode) {
                                                            if (arrCrbnkcode.length > 0) {
                                                                var Takeurl = `select api_url from CORE_API_MASTER_RULE_SETUP WHERE api_code = 'NPSSCreditTransferInitiation'`
                                                                ExecuteQuery1(Takeurl, function (arrurl) {
                                                                    if (arrurl.length > 0) {
                                                                       
                                                                        var url = arrurl[0]['api_url']
                                                                        var apiparam = {}
                                                                        apiparam.transaction_id = params.transaction_id
                                                                        apiparam.url = url
                                                                        ...
                                                                        fn_CallCoreBank(params.INTRBK_STTLM_AMNT,arrDrcustDtls[0].iban, function (result) {
                ...
                //coreBank call 
                function fn_CallCoreBank(Amount, iban,callbackapi) {
                    try {
                        var request = require('request');
                        var apiURL = "http://192.168.2.210:30509/microsvc/ibs_s_verify_reserve"
                        var options = {
                            url: apiURL,
                            timeout: 18000000,
                            method: 'POST',
                            json: {
                                ...
                                "routingkey": "CLT-1304~APP-200~TNT-adsrj~ENV-0",
                ...
                function fn_CallNestApi(apiparam, callbackapi) {
                    try {
                        var request = require('request');
                        var apiURL = apiparam.url
                        var options = {
                            url: apiURL,
                            timeout: 18000000,
                            method: 'POST',
                            json: {
                                "transactionId": apiparam.transaction_id,
                                ...
                                "recipient": {
                                    ...
                                    "mobile": apiparam.CDTR_PHONE_NO,

                                    "name": apiparam.CDTR_ACCT_NAME
                                ...
                                "sender": {
                                    ...
                                    "mobile": apiparam.DBTR_PHONE_NO,

                                    "name": apiparam.DBTR_ACCT_NAME
