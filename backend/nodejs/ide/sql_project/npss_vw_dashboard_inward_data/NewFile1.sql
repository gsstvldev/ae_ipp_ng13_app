...
CREATE OR REPLACE VIEW vw_dashboard_inward_data
 ...
 SELECT res.type,
   ...
   FROM ( SELECT
                ...
                END AS returned,
                CASE
                    WHEN nppst.process_status::text = 'RCTExceptionFailure'::text AND nppst.status::text = 'IP_RCT_RR_POSTING_FAILURE'::text OR nppst.status::text = 'IP_RCT_PC_T24_POSTING_FAILURE'::text OR nppst.status::text = 'IP_RCT_CC_T24_POSTING_FAILURE'::text THEN count(*)
                    WHEN nppst.process_status::text = 'RCTExceptionFailure'::text AND nppst.status::text = 'IP_RCT_RETURN_POSTING_FAILURE'::text THEN count(*)
                    ELSE NULL::bigint
                ...
                CASE
                    WHEN nppst.process_status::text = 'RCTExceptionFailure'::text AND (nppst.status::text = ANY (ARRAY['IP_RCT_RR_POSTING_RETRY'::text, 'IP_RCT_RETURN_POSTING_RETRY'::text, 'IP_RCT_RR_POSTING_SUSPICIOUS'::text])) THEN count(*)
                    ELSE NULL::bigint
