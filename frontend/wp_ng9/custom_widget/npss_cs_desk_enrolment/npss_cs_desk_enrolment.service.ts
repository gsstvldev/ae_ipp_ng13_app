...
@Injectable()
export class npss_cs_desk_enrolmentService {
    ...
    fn_npss_cs_desk_enrolment(source_id, destn_id, parent_source_id, event_code, event_params, screenInstance, internals, handler_code, event_data, data_source) {
        ...
        var CtrlScope = screenInstance['merchant_detail'].f_npss_merchant_ui.model;
         var CtrlScope_deskenroll = screenInstance['cashdesk_detail'].f_npss_cash_desk_ui.model;
        ...
        ClientParams = {
            ...
            "cashdesk_identification":CtrlScope_deskenroll.CASHDESK_OVERLAY_ID,
    ...
    CallUrlWithData(ClientParams, screenInstance, internals) {
        this.httpHelper.HttpPost('/microsvc/npss_cs_desk_enrolment/', ClientParams)
            .subscribe((res: any) => {
                var event

                if (res.data.status == 'SUCCESS') {

                    event = { eventId: "custom-connector", param: res.data.data, idename: "Deskenrolment", internals: internals }
