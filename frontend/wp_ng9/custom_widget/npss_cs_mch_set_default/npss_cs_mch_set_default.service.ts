...
@Injectable()
export class npss_cs_mch_set_defaultService {
    ...
    fn_npss_cs_mch_set_default(source_id,destn_id,parent_source_id,event_code,event_params,screenInstance,internals,handler_code,event_data,data_source){
    ...
    CallUrlWithData(ClientParams, screenInstance, internals) {
        this.httpHelper.HttpPost('/microsvc/npss_cs_mch_set_default/', ClientParams)
            .subscribe((res: any) => {
                var event

                if (res.data.status == 'SUCCESS') {

                    event = { eventId: "custom-connector", param: res.data.data, idename: "MerchantsetDefaultAcc", internals: internals }
