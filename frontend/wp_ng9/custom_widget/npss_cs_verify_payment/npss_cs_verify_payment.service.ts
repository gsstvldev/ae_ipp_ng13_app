...
@Injectable()
export class npss_cs_verify_paymentService {
    ...
    fn_npss_cs_verify_payment(source_id, destn_id, parent_source_id, event_code, event_params, screenInstance, internals, handler_code, event_data, data_source) {
    ...
    CallUrlWithData(ClientParams, screenInstance, internals) {
        this.httpHelper.HttpPost('/microsvc/npss_cs_verify_payment/', ClientParams)
            .subscribe((res: any) => {
                var event
                    ...
                    if (res.data.data) {
                        if (res.data.data.message) {
                            ...
                            if(res.data.data["result"]["code"] == '00000'){
                                this.coreHelper.set_value_to_memory("MEMORY_VARIABLES", "MI_LEVEL_PAYMENT_ID",res.data.data["payment"]["paymentId"]);
                            }
                            event = { eventId: "custom-connector", param: res.data.data, type:"Object", internals: internals }
                            screenInstance["retrieve_contact_cw"].onChangecomponent.emit(event);
                        }
                    } else {
                        this.dialogHelper.ShowInfoDialog('No Response from api')
                    }
