...
export class s_credit_advice_reportComponent implements OnInit,AfterViewInit {
	...
	cc_for_credit__internal(parent_event_result){
		this.cc_for_credit__se_for_pdf_btn(parent_event_result)
	}
	...
	search_search__cc_from_search() { 
		...
		let handler_code="custom_connectors"
		let internals="cc_from_search__ssp_from_search,cc_from_search__cp_from_nav_search,cc_from_search__cc_for_credit,"
		let event_data={}
		let data_source={}
		try {
	...
	//Handler for INTERNAL event of "cc from search"
	cc_from_search__cc_for_credit(parent_event_result) { 
		let Dest_Is_ctrl=true
		let parentEventResult ="SUCCESS"
	if(parentEventResult!=parent_event_result) return true;
		let source_id="cc_from_search"
		...
		let handler_code="custom_connectors"
		let internals="cc_for_credit__se_for_pdf_btn,"
		let event_data={}
		let data_source={}
		try {
			this.npss_cs_credit_debit_reportService.fn_npss_cs_credit_debit_report(source_id,destn_id,parent_source_id,event_code,event_params,this,internals,handler_code,event_data,data_source)
	...
	//Handler for INTERNAL event of "cc for credit"
	cc_for_credit__se_for_pdf_btn(parent_event_result) { 
		let Dest_Is_ctrl=true
		let parentEventResult ="SUCCESS"
	if(parentEventResult!=parent_event_result) return true;
		...
		let event_code="e_1712669526356"
		let event_params={"caller_name":"cc_for_credit__se_for_pdf_btn","event_desc":"SE For Pdf btn","event_type":"INTERNAL","caller_event_context":"SUCCESS","root_source_id":"search_search","raiseparam":{"visible_collapse_count":"","visible_collapse_locked_by":"","hide_for":"","hide_except":"","hide_column":"","expression":""},"parent_event_result":"SUCCESS"}
