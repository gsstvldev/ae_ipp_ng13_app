...
@Component({
	...
	providers:[torus_cs_show_hideService,npss_cs_find_linked_r_messagesService]
})
...
export class s_find_linked_r_messagesComponent implements OnInit,AfterViewInit {
	...
	// Constructor 
	constructor(private handler:AppHandlerService ,private torus_cs_show_hideService:torus_cs_show_hideService,private npss_cs_find_linked_r_messagesService:npss_cs_find_linked_r_messagesService) {
	...
	ve_for_save_to_api__internal(parent_event_result){
		this.ve_for_save_to_api__se_for_save_api(parent_event_result)
	}

	//Handler for INTERNAL event of "se for save api"
	se_for_save_api__internal(parent_event_result){
		this.se_for_save_api__inf_for_save_api(parent_event_result)
	}
	...
	api_input_ui_actions_save__ve_for_save_to_api() { 
		...
		let internals="ve_for_save_to_api__se_for_save_api,"
		let event_data={}
		let data_source={}
		try {
	...
	//Handler for INTERNAL event of "ve for save to api"
	ve_for_save_to_api__se_for_save_api(parent_event_result) { 
		let Dest_Is_ctrl=true
		let parentEventResult ="SUCCESS"
	if(parentEventResult!=parent_event_result) return true;
		...
		let event_code="e_1677334094905"
		let event_params={"caller_name":"ve_for_save_to_api__se_for_save_api","event_desc":"SE for Save API","event_type":"INTERNAL","caller_event_context":"SUCCESS","root_source_id":"api_input_ui_actions_save","raiseparam":{"reset_selection":"Y","need_clear_value":"Y","no_change_status_on_modify":"N","release_lock":"Y","clear_cache":"Y"},"parent_event_result":"SUCCESS"}
		let handler_code="save_tran"
		let internals="se_for_save_api__inf_for_save_api,"
		let event_data={}
		let data_source={}
		try {
	...
	//Handler for INTERNAL event of "se for save api"
	se_for_save_api__inf_for_save_api(parent_event_result) { 
		let Dest_Is_ctrl=true
		let parentEventResult ="SUCCESS"
	if(parentEventResult!=parent_event_result) return true;
		let source_id="se_for_save_api"
		let destn_id=""
		let parent_source_id="ve_for_save_to_api"
		let event_code="e_1677334132545"
		let event_params={"caller_name":"se_for_save_api__inf_for_save_api","event_desc":"INF for Save API","event_type":"INTERNAL","caller_event_context":"SUCCESS","message_text":"API Request has been saved successfully.","root_source_id":"api_input_ui_actions_save","raiseparam":{"info_msg":""},"parent_event_result":"SUCCESS"}
	...
	if(parentEventResult!=parent_event_result) return true;
		...
		let destn_id=""
		let parent_source_id="se_for_save_api"
		let event_code="e_1677334202199"
