...
export class s_home_dashboardComponent implements OnInit,AfterViewInit {
	...
	ngOnInit() {
		...
		this.outward.params = {}
		this.outward.datasource = {"default":{"dt_1304_1665901130705":{"dtt_1304_1665901217208":{"st_ds":{"default":{"uicgc_code":"UICGC_30","event_code":"DEFAULT","dt_code":"DT_1304_1665901130705","dt_desc":"NPSS EDGE Transactions Group","dtt_code":"DTT_1304_1665901217208","dtt_desc":"NPSS Trn Process Log","ds_eligible":"DS_1681970564121","ds_final":"","final_state":"","property_json":{"columns":{}},"system_type":"DEFAULT","eq_info":{"eq_code":"NPSS Home Dashboard OP BAR Chart MQ","eq_type":"S","eq_mode":"M","eq_designer_json":{},"eq_json":{"columns":[{"column_name":"Pending_Detais","alias_name":"","mq_id":"MQ_1681908553128","date_format":false},{"column_name":"p_count","alias_name":"","mq_id":"MQ_1681908553311","date_format":false}],"joins":[]},"eq_text":"select  X.Pending_Detais,  x.p_count,  X.TYPE from  (  select   A.type,   case    when A.type = 'pacs.008'    and b.sno is null then 1    when A.type = 'pacs.004'    and b.sno is null then 2    when A.type = 'pacs.007'    and b.sno is null then 3    else b.sno   end sno,   coalesce(b.total, 0) as total,   coalesce(b.IBMB, 0) as IBMB,   coalesce(b.RIB, 0) as RIB,   coalesce(b.RMB, 0) as RMB,   coalesce(b.CIB, 0) as CIB,   coalesce(b.CMS, 0) as CMS,   coalesce(b.CMB, 0) as CMB,   coalesce(b.NAF, 0) as NAF,   coalesce(b.Manual, 0) as Manual,   coalesce(b.Pending_Screening, 0) as Pending_Screening,   coalesce(b.PENDING_MAKER, 0) as PENDING_MAKER,   coalesce(b.PENDING_CHECKER, 0) as PENDING_CHECKER,   coalesce(b.SEND_TO_CB, 0) as SEND_TO_CB,   coalesce(b.CBACK, 0) as CBACK,   coalesce(b.CBNACK, 0) as CBNACK,   coalesce(b.Pending_T_1, 0) as Pending_T_1,   to_char(CURRENT_DATE::timestamp with time zone, 'yyyy-mm-dd'::text)::timestamp without time zone as created_date  from   (   select    'pacs.008' as type  union   select    'pacs.004' as type  union   select    'pacs.007' as type) A  left join (   select    SNO,    type,    SUM(TOTAL) as Total,    SUM(IBMB) as IBMB,    SUM(RIB) as RIB,    SUM(RMB) as RMB ,    SUM(CIB) as CIB,    SUM(CMS) as CMS,    SUM(CMB) as CMB,    SUM(NAF) as NAF,    SUM(MANUAL) as Manual,    SUM(PENDING_SCREENING) as Pending_Screening,    SUM(PENDING_MAKER) as Pending_Maker,    SUM(PENDING_CHECKER) as Pending_Checker,    SUM(SEND_TO_CB) as Send_to_CB,    SUM(CBACK) as CBACK,    SUM(CBNACK) as CBNACK,    CREATED_DATE,    SUM(PENDING_T_1) as Pending_T_1   from    (    select     SNO,     type,     Total,     IBMB,     RIB,     RMB ,     CIB,     CMS,     CMB,     NAF,     MANUAL,     PENDING_SCREENING,     PENDING_MAKER,     PENDING_CHECKER,     SEND_TO_CB,     CBACK,     CBNACK,     CREATED_DATE,     PENDING_T_1,     DEPARTMENT_CODE,     TENANT_ID    from     vw_dashboard_outward_data) as t $WHERE   group by    SNO,    type,    CREATED_DATE) B on   B.type = A.type  order by   sno ) AB cross join lateral( values ('Total', AB.TOTAL, AB.TYPE), ('RIB', AB.RIB, AB.TYPE), ('IBMB', AB.IBMB, AB.TYPE), ('RMB', AB.RMB, AB.TYPE), ('CIB', AB.CIB, AB.TYPE), ('CMS', AB.CMS, AB.TYPE), ('CMB', AB.CMB, AB.TYPE), ('NAF', AB.NAF, AB.TYPE), ('Manual', AB.Manual, AB.TYPE), ('Pending (Checker)', AB.PENDING_CHECKER, AB.TYPE), ('Pending Screening', AB.Pending_Screening, AB.TYPE), ('SEND TO CB', AB.SEND_TO_CB, AB.TYPE), ('CBACK', AB.CBACK, AB.TYPE), ('CBNACK', AB.CBNACK, AB.TYPE), ('Pending (T-1)', AB.Pending_T_1, AB.TYPE), ('Pending (Maker)', AB.PENDING_MAKER, AB.TYPE)) X ( Pending_Detais,  p_count,  type ) where  p_count not in ('0')"},"listing_mode":"","locking_mode":"","locking_parameter":"","ccd_name":"NPSS Home Dashboard OP BAR Chart CCD","filter":[{"filter_name":"DEPARTMENT_CODE","binding_name":"DEPARTMENT_CODE","binding_value":"","source_name":"SYSTEM_EXTENDED_INFO.department_code","source_value":"","source_type":"SESSION_LEVEL","oprtr":"IN","data_type":"TEXT","conj_operator":"","group_no":""},{"filter_name":"TENANT_ID","binding_name":"TENANT_ID","binding_value":"","source_name":"TENANT_ID","source_value":"","source_type":"SESSION_LEVEL","oprtr":"=","data_type":"TEXT","conj_operator":"AND","group_no":""},{"filter_name":"TYPE","binding_name":"TYPE","binding_value":"","source_name":"MI_LEVEL_TYPE","source_value":"","source_type":"MI_LEVEL","oprtr":"=","data_type":"TEXT","conj_operator":"AND","group_no":""}],"databinding":[{"header":"Pending_Detais","target_column":"Pending_Detais","alias_name":"","alignment":"Left","width":"","format":"","date_format":false},{"header":"p_count","target_column":"p_count","alias_name":"","alignment":"Left","width":"","format":"","date_format":false}]}}}}}}
