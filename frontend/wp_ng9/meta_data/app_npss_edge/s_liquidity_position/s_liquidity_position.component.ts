...
export class s_liquidity_positionComponent implements OnInit,AfterViewInit {
	...
	ve_for_save_to_api__internal(parent_event_result){
		this.ve_for_save_to_api__save_tran_for_lp(parent_event_result)
	}

	//Handler for INTERNAL event of "save tran for lp"
	save_tran_for_lp__internal(parent_event_result){
		this.save_tran_for_lp__info_for_lp(parent_event_result)
	}

	//Handler for INTERNAL event of "info for lp"
	info_for_lp__internal(parent_event_result){
		this.info_for_lp__rs_for_lp(parent_event_result)
	}
	...
	api_input_ui_actions_save__ve_for_save_to_api() { 
		...
		let internals="ve_for_save_to_api__save_tran_for_lp,"
		let event_data={}
		let data_source={}
		try {
	...
	//Handler for INTERNAL event of "ve for save to api"
	ve_for_save_to_api__save_tran_for_lp(parent_event_result) { 
		let Dest_Is_ctrl=true
		let parentEventResult ="SUCCESS"
	if(parentEventResult!=parent_event_result) return true;
		...
		let event_code="e_1677334215306"
		let event_params={"caller_name":"ve_for_save_to_api__save_tran_for_lp","event_desc":"save tran for LP","event_type":"INTERNAL","caller_event_context":"SUCCESS","root_source_id":"api_input_ui_actions_save","raiseparam":{"reset_selection":"Y","need_clear_value":"Y","no_change_status_on_modify":"N","release_lock":"Y","clear_cache":"Y"},"parent_event_result":"SUCCESS"}
		let handler_code="save_tran"
		let internals="save_tran_for_lp__info_for_lp,"
		let event_data={}
		let data_source={}
		try {
	...
	//Handler for INTERNAL event of "save tran for lp"
	save_tran_for_lp__info_for_lp(parent_event_result) { 
		let Dest_Is_ctrl=true
		let parentEventResult ="SUCCESS"
	if(parentEventResult!=parent_event_result) return true;
		let source_id="save_tran_for_lp"
		let destn_id=""
		let parent_source_id="ve_for_save_to_api"
		let event_code="e_1677334237609"
		let event_params={"caller_name":"save_tran_for_lp__info_for_lp","event_desc":"INFO FOR LP","event_type":"INTERNAL","caller_event_context":"SUCCESS","message_text":"API Request has been saved successfully.","root_source_id":"api_input_ui_actions_save","raiseparam":{"info_msg":""},"parent_event_result":"SUCCESS"}
		let handler_code="info_msg"
		let internals="info_for_lp__rs_for_lp,"
		let event_data={}
		let data_source={}
		try {
	...
	//Handler for INTERNAL event of "info for lp"
	info_for_lp__rs_for_lp(parent_event_result) { 
		let Dest_Is_ctrl=true
		let parentEventResult ="SUCCESS"
	if(parentEventResult!=parent_event_result) return true;
		let source_id="info_for_lp"
		let destn_id=""
		let parent_source_id="save_tran_for_lp"
		let event_code="e_1677334266104"
		let event_params={"caller_name":"info_for_lp__rs_for_lp","event_desc":"RS for LP","event_type":"INTERNAL","caller_event_context":"SUCCESS","root_source_id":"api_input_ui_actions_save","raiseparam":{},"parent_event_result":"SUCCESS"}
