...
Build ID        : 28203 
Modified By     : Admin 
Modified Date   : 2023-Feb-20 14:42 PM 
...
export class s_op_view_dashboardComponent implements OnInit,AfterViewInit {
	...
	ngOnInit() {
		...
		this.dashboard.datasource = {"default":{"dt_1304_1665901130705":{"dtt_1304_1665905039255":{"st_ds":{"default":{"uicgc_code":"UICGC_19","event_code":"DEFAULT","dt_code":"DT_1304_1665901130705","dt_desc":"NPSS EDGE Transactions Group","dtt_code":"DTT_1304_1665905039255","dtt_desc":"NPSS Trn Process Log","ds_eligible":"DS_1672920886459","ds_final":"","final_state":"","property_json":{"columns":{}},"system_type":"DEFAULT","eq_info":{"eq_code":"NPSS OP PL DASHBOARD MQ","eq_type":"S","eq_mode":"M","eq_designer_json":{},"eq_json":{"columns":[{"column_name":"RESPONSE_DETAIL","alias_name":"","mq_id":"MQ_1672915674534","date_format":false},{"column_name":"RESPONSE_DETAIL_COUNT","alias_name":"","mq_id":"MQ_1672915675846","date_format":false}],"joins":[]},"eq_text":"select  COUNT (RESPONSE_DETAIL) as RESPONSE_DETAIL_COUNT,  RESPONSE_DETAIL,  RESPONSE_DETAIL_ORDER from  (  select   case    when response_code = UPPER('ACCP')    and cbuae_return_code is null then 'ACK response received from CB'    when response_code = UPPER('RJCT')    and cbuae_return_code is null then 'NAK Response received from CB'    when response_code = UPPER('ACCP')    and cbuae_return_code is not null    and process_name not in ('Receive pacs004') then 'Accepted by Bene Bank'    when response_code = UPPER('RJCT')    and cbuae_return_code is not null then 'Rejected by Bene'    when ntpl.status in (UPPER('OP_P2P_POSTING_SUCCESS'), UPPER('OP_P2B_FUND_AUTHORIZED')) then 'Posting Completed'    when NT.process_status in ('RCTCompleted','RCTReturned') and NTPL.process_name = 'Place Pacs008' then 'Send to CB'    when ntpl.status in (UPPER('OP_AC_STATUS_ACCEPTED'), UPPER('OP_P2P_STATUS_ACCEPTED'), UPPER('OP_P2B_STATUS_ACCEPTED')) then 'CB ACK'    when ntpl.status in (UPPER('OP_AC_REV_POSTING_SUCCESS'), UPPER('OP_AC_STATUS_REJECTED'), UPPER('OP_P2B_STATUS_REJECTED'),UPPER('OP_P2B_REV_POSTING_SUCCESS'),UPPER('OP_P2P_STATUS_REJECTED'),UPPER('OP_P2P_REV_POSTING_SUCCESS')) then 'CB NACK'    else initcap('Pending')   end as RESPONSE_DETAIL,   case    when response_code = UPPER('ACCP') and cbuae_return_code is null then 1    when response_code = UPPER('RJCT') and cbuae_return_code is null then 2    when response_code = UPPER('ACCP') and cbuae_return_code is not null and process_name not in ('Receive pacs004') then 3    when response_code = UPPER('RJCT') and cbuae_return_code is not null then 4    when ntpl.status in (UPPER('OP_P2P_POSTING_SUCCESS'), UPPER('OP_P2B_FUND_AUTHORIZED')) then 5    when nt.process_status in ('RCTCompleted','RCTReturned') and NTPL.process_name = 'Place Pacs008' then 6    when ntpl.status in (UPPER('OP_AC_STATUS_ACCEPTED'), UPPER('OP_P2P_STATUS_ACCEPTED'), UPPER('OP_P2B_STATUS_ACCEPTED')) then 7    when ntpl.status in (UPPER('OP_AC_REV_POSTING_SUCCESS'), UPPER('OP_AC_STATUS_REJECTED'), UPPER('OP_P2B_STATUS_REJECTED'),UPPER('OP_P2B_REV_POSTING_SUCCESS'),UPPER('OP_P2P_STATUS_REJECTED'),UPPER('OP_P2P_REV_POSTING_SUCCESS')) then 8    else 19   end as RESPONSE_DETAIL_ORDER,   case    when ntpl.status like 'OP%' then 'OP'   end as PROCESSTYPE,   NT.CREATED_DATE,   NT.TENANT_ID,   NT.department_code,   NT.process_type,   NT.uetr   from   npss_trn_process_log ntpl  inner join npss_transactions nt on   NT.uetr = NTPL.uetr) G $WHERE group by  RESPONSE_DETAIL_ORDER,  RESPONSE_DETAIL order by  RESPONSE_DETAIL_ORDER"},"listing_mode":"","locking_mode":"","locking_parameter":"","ccd_name":"NPSS OP PL Dashboard CCD","filter":[{"filter_name":"DEPARTMENT_CODE","binding_name":"DEPARTMENT_CODE","binding_value":"","source_name":"SYSTEM_EXTENDED_INFO.department_code","source_value":"","source_type":"SESSION_LEVEL","oprtr":"IN","data_type":"TEXT","conj_operator":"AND","group_no":""},{"filter_name":"PROCESS_TYPE","binding_name":"PROCESS_TYPE","binding_value":"","source_name":"OP","source_value":"","source_type":"HARDCODED","oprtr":"=","data_type":"TEXT","conj_operator":"","group_no":""},{"filter_name":"RESPONSE_DETAIL","binding_name":"RESPONSE_DETAIL","binding_value":"","source_name":"Pending","source_value":"","source_type":"HARDCODED","oprtr":"<>","data_type":"TEXT","conj_operator":"AND","group_no":""},{"filter_name":"TENANT_ID","binding_name":"TENANT_ID","binding_value":"","source_name":"TENANT_ID","source_value":"","source_type":"SESSION_LEVEL","oprtr":"=","data_type":"TEXT","conj_operator":"AND","group_no":""}],"databinding":[{"header":"Detail","target_column":"RESPONSE_DETAIL","alias_name":"","alignment":"Left","width":"","format":"","date_format":false},{"header":"Detail Count","target_column":"RESPONSE_DETAIL_COUNT","alias_name":"","alignment":"Left","width":"","format":"","date_format":false}]}}}}}}
